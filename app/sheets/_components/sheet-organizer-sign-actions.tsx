...
type ShareChannel = 'copy' | 'viber' | 'telegram' | 'whatsapp';
...
type SheetOrganizerSignActionsProps = {
  ...
  shareAction: (state: SheetFormState, formData: FormData) => Promise<SheetFormState>;
...
export default function SheetOrganizerSignActions({
  ...
  const [shareState, shareFormAction, sharePending] = useActionState(shareAction, initialState);
  ...
  const pendingShareChannelRef = useRef<ShareChannel | null>(null);
  const shareMenuRef = useRef<HTMLDivElement | null>(null);
  const shareFormId = `sheet-organizer-share-${sheetId}`;

  const isPending = signPending || sharePending || refreshPending;
  const error = signState.error ?? shareState.error ?? refreshState.error;
  ...
  useEffect(() => {
    ...
    const channel = pendingShareChannelRef.current;
    if (!channel) {
      return;
    }

    pendingShareChannelRef.current = null;
    const signingUrl = shareState.signingUrl;
    if (!signingUrl) {
      return;
    }

    if (channel === 'copy') {
      void copySigningLink(signingUrl);
      return;
    }

    const shareUrl = getShareUrl(channel, signingUrl);
    window.location.href = shareUrl;
  }, [sharePending, shareState.signingUrl]);

  function getShareUrl(channel: Exclude<ShareChannel, 'copy'>, fullUrl: string) {
  ...
  function handleShareOption(channel: ShareChannel) {
    pendingShareChannelRef.current = channel;
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        <form id={shareFormId} action={shareFormAction} className="hidden">
        ...
        <div ref={shareMenuRef} className="relative inline-flex">
          <Button
            type="button"
            variant="outline"
            className="hover:bg-background h-8 px-3 text-xs"
            disabled={disabled || isPending}
            onClick={() => setIsShareMenuOpen((isOpen) => !isOpen)}
            title="Поділитись посиланням на підпис"
            aria-haspopup="menu"
          ...
          >
            ...
            {sharePending ? 'Готуємо посилання...' : 'Поділитись'}
          </Button>
          {isShareMenuOpen ? (
            <div
              role="menu"
              aria-label="Поділитись посиланням на підпис"
              className="border-brand/40 absolute top-full left-0 z-10 mt-1 min-w-44 rounded-md border bg-[#F7F8FA] p-1 shadow-sm"
            >
              <button
                type="submit"
                role="menuitem"
                form={shareFormId}
                disabled={disabled || isPending}
                className="focus-visible:ring-ring focus-visible:ring-offset-background hover:text-brand flex w-full cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs font-medium transition-colors duration-150 hover:bg-[#F6F5FF] focus-visible:bg-[#F6F5FF] focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60"
